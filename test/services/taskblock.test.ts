...
import { isValidMultipleChoiceAnswer, quizBlockContainsQuestionIdWithPrefix} from "../../src/services/questionHelper";
...
describe("Validating a question answer submission", () => {
   it("will make sure the question is contained in the provided quizblock as the expected type", async () => {
      ...
      expect(quizBlockContainsQuestionIdWithPrefix(quizBlock, question.id, "MC_QUESTION")).toBeTruthy();
      
      // exists, but wrong type 
      expect(quizBlockContainsQuestionIdWithPrefix(quizBlock, question.id, "FR_QUESTION")).toBeFalsy();

      // doesn't exist at all
      expect(quizBlockContainsQuestionIdWithPrefix(quizBlock, "NON_EXISTENT_ID", "MC_QUESTION")).toBeFalsy();
   });
   ...
   it("will make sure the provided answer is an option for a multiple choice question", async () => {
      const question: MCQuestion = {
         ...
         description: "",
         points: 1,
         options: [{id: 0, description: "O1"}, {id: 1, description: "O2"}],
         answers: [1]
      };
